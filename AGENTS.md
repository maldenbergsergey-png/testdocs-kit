...
- Use the skill in `skills/` that matches the requested task: derive a standard from a corpus, build a coverage matrix, generate, build a regression model, analyze coverage, propose updates, or review.
...
- Keep edits focused on rules, skills, examples, agent guidance, or integration documentation. Do not add an app, service, database, model dependency, or orchestration runtime.
