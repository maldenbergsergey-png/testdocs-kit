...
- Route by user intent; never require the user to know a skill name. Checklist-only, full package, cases-only, task-scoped non-regression cases, and targeted-scope requests route through `prepare-task-testing`. Bug-report drafting or Jira bug creation uses `create-bug-report`. Generate-only uses `generate-test-cases`; optimization/refactoring or review of supplied cases uses `review-test-cases`; a specific-case apply/update uses `update-test-cases`; coverage-only uses `analyze-test-coverage`; matrix uses `build-coverage-matrix`; regression organization uses `build-regression-model`; corpus/rule derivation uses `derive-test-case-standard`. A Jira issue, Confluence page, standalone document, file, or chat requirement can each be the scope source. Direct skill calls remain supported.
...
- For bug reports, read the exact project's Jira create metadata before a create operation. Use dedicated semantic fields when present and fall back to a structured Description without duplication. Keep reporter/author and assignee on the authenticated tester by default; use the same user for an unambiguous specialist field. Do not guess optional labels, components, ownership, priority, or other routing.
