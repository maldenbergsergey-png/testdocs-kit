...
test("existing-case updates reject even explicit approval and a complete fingerprint without any network request", async (t) => {
  const fetchMock = t.mock.method(global, "fetch", async () => response(200, { key: "UNEXPECTED-T1" }));
  for (const input of [
    { confirmed: true, testCaseKey: "DEMO-T20", expectedBaselineHash: "a".repeat(64), name: "Approved proposal" },
    { testCaseKey: "DEMO-T20", name: "Unconfirmed" }
  ]) {
    await assert.rejects(tools.zephyr_update_test_case(input), { code: "SESSION_ONLY" });
  }
  assert.equal(fetchMock.mock.callCount(), 0);
});
