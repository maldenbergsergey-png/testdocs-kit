...
Upload only the files selected above using binary PUT requests. A renewed session or a corrected/new batch may require uploading those bytes again because temporary server files are released; this is file delivery, not a new test run. Do not revisit the tested application merely to recreate attachments. Reuse one upload within a batch when the same file supports multiple rows. After `saved`, inspect the QA Report cells when browser viewing is available to check that images load under the actual results and small text evidence renders as code; report any visual verification limit. Apply the manual-Jira-publication boundary from `checklist-delivery-rules.md` when that is the user's chosen handoff.
