...
Reading context does not authorize writing. A request only to analyze, draft, generate, update, or review does not authorize publication. An explicit request to create or publish new cases may authorize same-turn creation after validation. Cases created outside the current MCP process remain proposal-only under [the temporary update restriction](../rules/update-rules.md). Only registry-confirmed session corrections may be applied after an explicit request. Destructive operations are not exposed.
