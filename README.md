...
Инструмент работает как intent-based QA-assistant: получает задачу, документ, ссылку, файл или тест-кейсы, сам выбирает workflow и возвращает только запрошенный артефакт. Он умеет формировать Jira-чек-листы, полный пакет документации, отдельные и task-only кейсы, проводить review и оптимизировать существующий набор. Имена skills пользователю знать не нужно.
...
```text
Напиши тест-кейсы по приложенному документу. Чек-лист не нужен.
```
...
### 4. Только тест-кейсы для задачи

```text
Напиши тест-кейсы для TASK-123. Чек-лист не нужен.
```
...
```text
Для TASK-123 напиши только тест-кейсы блока «Фильтры». Чек-лист не нужен.
```
...
Обычное «Подготовь тестирование для …» запускает полный пакет. Явные слова «только», «без чек-листа», «не для регресса» и название конкретного блока ограничивают scope. Jira, Confluence и TMS необязательны: те же сценарии работают с текстом и файлами, переданными вручную. Подробнее — в [описании workflows](docs/workflows.md).
