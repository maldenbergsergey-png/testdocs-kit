...
9. Отдельно разрешить или оставить выключенным создание багов Jira по явному запросу.
10. Отдельно разрешить или оставить выключенной публикацию checklist-комментариев в Jira.
11. При необходимости указать адрес QA Report и отдельно разрешить отправку checklist в его редактор.
...
9. По отдельному opt-in подключает создание одного Jira Bug после чтения живой схемы полей и явного запроса пользователя.
10. По отдельному opt-in подключает публикацию готового checklist в Jira и импорт в QA Report; произвольные комментарии и transitions остаются выключенными.
11. При необходимости подключает дополнительный доверенный CA-бандл, не отключая TLS-проверку.
12. Для browser-session режима Jira/Confluence открывает отдельный профиль Chrome/Edge/Chromium и сохраняет подтверждённую сессию.
...
```text
Jira MCP: 15 инструментов (при выбранном Zephyr; чтение + создание и защищённое обновление кейсов по явному запросу)
