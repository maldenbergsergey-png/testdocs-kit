...
Количество может измениться после добавления новых инструментов. Из write-операций доступны `zephyr_create_test_case`, `zephyr_update_session_test_case` и `zephyr_update_test_case`. Последний требует fingerprint полного baseline и отклоняет stale proposal. Новая версия, перенос, комментарий, изменение статуса и удаление не регистрируются.
...
- Установщик всегда записывает `enableWrites: false`; версии, переносы, комментарии, смена статуса, links и другие общие write-операции не регистрируются.
