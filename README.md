...
Инструмент помогает писать простые и понятные тест-кейсы, проверять покрытие, предлагать актуализацию существующих кейсов и формировать переиспользуемую регрессионную модель. Он также умеет создать новый кейс по явному запросу и исправить его в той же запущенной MCP-сессии.
...
7. Подключает создание новых Zephyr/TM4J-кейсов и исправление только что созданного кейса только по явному запросу; остальные write-инструменты оставляет выключенными.
...
```text
Jira MCP: 13 инструментов (чтение + создание и исправление кейса текущей сессии по явному запросу)
Confluence MCP: 5 read-only инструментов
Проверка установки: OK
```

Количество может измениться после добавления новых инструментов. Из write-операций доступны только `zephyr_create_test_case` и `zephyr_update_session_test_case`. Вторая операция принимает исключительно ключ, который этот же MCP-процесс получил при создании кейса после своего запуска. Произвольное обновление, новая версия, перенос, комментарий, изменение статуса и удаление не регистрируются.
...
| [`update-test-cases`](skills/update-test-cases/SKILL.md) | Предложить изменение существующего кейса или исправить кейс, созданный текущей MCP-сессией. |
...
- Установщик всегда записывает `enableWrites: false`; общие обновления, версии, переносы, комментарии, смена статуса и другие write-операции не регистрируются.
...
- Создание реализовано через совместимый Server/Data Center endpoint `POST /rest/atm/1.0/testcase`, а исправление кейса текущей сессии — через `PUT /rest/atm/1.0/testcase/{key}`. При исправлении шагов передаётся полный итоговый список: отсутствующие шаги удаляются, а шаги без ID создаются заново. На конкретной старой установке обе операции необходимо проверить одним демонстрационным кейсом.
- Редактирование ранее существовавших, найденных или созданных в прошлой MCP-сессии кейсов намеренно заблокировано.
...
- Ранее существующие, найденные и созданные в прошлой сессии кейсы остаются read-only. Применить исправление можно только к кейсу, который создал текущий MCP-процесс.
