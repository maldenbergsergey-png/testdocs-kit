...
```text
Подготовь чек-лист для тестирования TASK-123.
```

```text
Подготовь полный набор тестовой документации для TASK-123.
```
...
```text
Применить предложенные изменения в PROJECT-T123.
```
...
5. Выбрать TMS: Zephyr Scale / Test Management for Jira либо QA Tools (ТестОпс).
6. Для Zephyr не вводить отдельный адрес или токен: он использует подключение Jira. Для QA Tools указать адрес инстанса и выбрать API-токен либо логин с паролем.
7. При желании отдельно разрешить изменяющие QA Tools tools; удаление остаётся недоступным.
8. Аналогично настроить Confluence либо пропустить его.
9. Отдельно разрешить или оставить выключенной публикацию checklist-комментариев в Jira.
10. При необходимости указать адрес QA Report и отдельно разрешить отправку checklist в его редактор.
...
1. Устанавливает зависимости Jira MCP, Confluence MCP и выбранного QA Tools proxy.
...
4. Сохраняет Jira, Confluence и выбранную TMS в отдельном пользовательском файле.
...
9. По отдельному opt-in подключает публикацию готового checklist в Jira и импорт в QA Report; произвольные комментарии и transitions остаются выключенными.
10. При необходимости подключает дополнительный доверенный CA-бандл, не отключая TLS-проверку.
11. Для browser-session режима Jira/Confluence открывает отдельный профиль Chrome/Edge/Chromium и сохраняет подтверждённую сессию.
...
Для созданных и прочитанных кейсов Jira MCP возвращает полный адрес вида `https://jira.company.example/secure/Tests.jspa#/testCase/PROJECT-T123`. Если конкретная установка использует другой UI-маршрут, измените только `jira.testCaseUrlTemplate` в закрытом `~/.config/testdocs-kit/config.json`. Шаблон должен быть полным HTTP(S)-адресом и содержать `{key}`; дополнительно поддерживается `{projectKey}`. После изменения перезапустите AI-клиент.
...
В файле находятся STDIO-конфигурации выбранных подключений: Jira, Confluence, QA Tools и QA Report добавляются только когда включены. Скопируйте нужные записи в MCP-настройки своего клиента.
...
```text
Jira MCP: 14 инструментов (при выбранном Zephyr; чтение + создание и защищённое обновление по явному запросу)
...
Если выбран QA Tools, локальная проверка подтверждает конфигурацию и код proxy без обращения к внешнему инстансу. Полный удалённый handshake и актуальный список `testops_*` tools выполняются клиентом после перезапуска.

При выбранном Zephyr доступны защищённые Zephyr tools и, только после отдельного opt-in, `jira_publish_checklist_comment` и `qa_report_import_checklist`. Existing-case update требует fingerprint полного baseline и отклоняет stale proposal. При выбранном QA Tools proxy получает актуальные `testops_*` tools от инстанса, скрывает изменяющие операции без opt-in и всегда скрывает удаления.
