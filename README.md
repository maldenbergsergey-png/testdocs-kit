...
7. Подключает создание новых Zephyr/TM4J-кейсов только по явному запросу; обновление и остальные write-инструменты оставляет выключенными.
...
```text
Jira MCP: 12 инструментов (чтение + создание кейса по явному запросу)
Confluence MCP: 5 read-only инструментов
Проверка установки: OK
```

Количество может измениться после добавления новых инструментов. Разрешён только `zephyr_create_test_case`; обновление, версия, перенос, изменение статуса и удаление кейсов не регистрируются.
...
- Установщик всегда записывает `enableWrites: false`; обновление, версия, перенос, комментарии, смена статуса и другие write-операции не регистрируются.
...
- Создание реализовано через совместимый Server/Data Center endpoint `/rest/atm/1.0/testcase`. На конкретной старой установке его необходимо проверить одним демонстрационным кейсом; обновление существующих кейсов намеренно не реализовано.
