...
- Встроенный Jira MCP пока не предоставляет универсальные операции создания Test Run/Test Cycle, назначения executions и создания произвольной QA-задачи типа, отличного от Bug. Новый skill полностью формирует и валидирует proposal и выполняет запись только при наличии подключённого адаптера с этими отдельными capabilities; отсутствие capability не маскируется как успешное создание.
