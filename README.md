...
При выбранном Zephyr доступны read-tools для кейсов и их связей с Jira-задачами, защищённые case tools и, только после отдельных opt-in, `jira_create_bug`, `jira_publish_checklist_comment`, `zephyr_create_test_run`, `jira_create_qa_work_item` и `qa_report_import_checklist`. Existing-case update требует fingerprint полного baseline и отклоняет stale proposal. При выбранном QA Tools proxy получает актуальные `testops_*` tools от инстанса, скрывает изменяющие операции без opt-in и всегда скрывает удаления.
