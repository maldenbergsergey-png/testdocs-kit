...
Для запроса "протестируй мобильное приложение" агент сначала проверяет среду нужной платформы и доступные устройства. Если среды нет, анализирует предоставленные скринкасты и скриншоты по условиям задачи, указывает источник доказательств и отмечает непоказанную часть. Мобильный сайт проверяется браузером.

Для управления приложением нужны Maestro CLI и Java 17+. Android-среда - Android Studio/Android SDK с настроенными эмуляторами; iOS-среда на Mac - Xcode с Simulator. Агент выбирает подходящее устройство, а пользователь по умолчанию устанавливает нужную тестовую сборку и открывает приложение. После передачи приложения агент проверяет его через Maestro MCP: нажимает элементы, вводит текст, сверяет результат с требованиями и сохраняет скриншоты. Если приложение уже подготовлено, повторная установка не нужна. [Подключение и ограничения](integrations/profiles/maestro.md).
...
Агент собирает требования и доступный контекст, готовит рабочий checklist, выполняет доступные web/API/integration проверки, сравнивает интерфейс с переданным Figma-узлом и формирует Jira Wiki отчёт с фактическими результатами и доказательствами. Недоступные browser/mobile/log проверки остаются явно отмеченными, а не считаются выполненными.
