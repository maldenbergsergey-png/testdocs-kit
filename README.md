...
- для Eva — единый адрес инстанса, созданный в Eva API-токен и установленный `evateamclient-mcp` либо одобренный компанией совместимый MCP;
...
4. Для Eva ввести один адрес всего инстанса, API-токен и команду Eva MCP. Этот MCP предоставляет задачи EvaProject и документы EvaWiki через одно подключение.
...
1. Устанавливает зависимости Jira MCP, Confluence MCP и безопасных proxy для QA Tools/Eva.
...
9. Для Eva запускает отдельное MCP-подключение на каждый инстанс и публикует только read-only tools задач, комментариев, документов, пользователей и связей. Изменение, архивирование и удаление данных Eva скрыты.
...
Перед настройкой установите Go и сторонний Eva MCP, если компания не выдала собственную совместимую команду:

```bash
go install github.com/raoptimus/evateamclient.go/pkg/evateamclient-mcp@latest
```

Убедитесь, что каталог Go binaries добавлен в `PATH`, затем выполните `npm run configure:eva` или `npm run add:eva`. По умолчанию ожидается команда `evateamclient-mcp`; установщик позволяет сохранить другой полный путь.

Локальный proxy Testdocs Kit пропускает только чтение задач EvaProject, комментариев, проектов, документов EvaWiki, пользователей, типов задач, тегов и связей. Сторонние create/update/delete/archive tools не передаются AI-клиенту. Создание багов в Eva пока не поддерживается: текущий MCP не предоставляет надёжный аналог live create metadata для разных форм и пользовательских полей.
...
В обычном режиме проверка запускает настроенные MCP и проверяет их handshake; содержимое рабочих задач и страниц не запрашивается. В тестовом `--no-cli` режиме внешние Eva/QA Tools handshake пропускаются и проверяется только локальный proxy. Полный актуальный список внешних tools в этом случае будет получен клиентом после перезапуска.
