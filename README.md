...
Для запроса "протестируй мобильное приложение" агент по умолчанию использует доступный локальный Maestro MCP: осматривает экран, нажимает элементы, вводит текст, проверяет результат и сохраняет скриншоты в отчёт. Мобильный сайт проверяется браузером.

Подготовьте Maestro CLI и Java 17+. Для Android нужны Android SDK/эмулятор и тестовая APK; для iOS на Mac - Xcode с Simulator и сборка `.app` для симулятора. [Подключение и ограничения](integrations/profiles/maestro.md).
