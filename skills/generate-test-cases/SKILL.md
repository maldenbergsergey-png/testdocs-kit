...
8. Identify supported direction, theme, object-type, or presentation differences. Draft a delta-only variant case whose first called step links the base functional case; do not repeat common steps or create variants without a supported difference. Use direction names only from the current project's supplied context under `project-conventions.md`.
