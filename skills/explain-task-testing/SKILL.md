...
- [`../../rules/task-explanation-rules.md`](../../rules/task-explanation-rules.md)
...
Use [`../collect-test-context/SKILL.md`](../collect-test-context/SKILL.md) when source content behind an anchored Jira/GitLab issue, Figma node, knowledge page or another external link needs retrieval.
...
Do not output Jira Wiki tables, permanent Test Cases, statuses or fabricated results. If the user changes the request to actual execution, route to `execute-task-testing`. If they ask for a checklist, select `generate-test-checklist`; for Test Cases, select `generate-test-cases` directly. Preserve the requested scope.
