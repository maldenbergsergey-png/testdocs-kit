...
8. Run the language, completeness, decomposition, provenance, and Jira Wiki self-check from the standard on the final serialized checklist, including expected results and comments. Validate every row against the column positions specified there, including rows with source comments; use the format validator described in the standard when available.
