...
- Verify content, attachments, task link, and environment separately before reporting completion. For the temporary API, only a `saved` batch receipt confirms content/attachment persistence; verify metadata through a supported read or the editor. Preserve successful batch IDs on partial failure and use the profile's idempotent retry contract.
