...
- **TMS completeness:** the case uses the exact Russian Zephyr fields and order from `test-case-standard.md`; step-level test data and setup/source hints relate to the current action, a cell with neither is empty, and postconditions are explicit even when they are `Не требуются`.
