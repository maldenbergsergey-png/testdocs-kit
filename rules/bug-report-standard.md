...
- Backend, logical, or server defect: minimal safe request context and the actual unmodified response under [response authenticity](execution-evidence-rules.md#реальные-ответы-бэкэнда). Include a small body in full inline; attach a large original body with a verbatim excerpt when useful. Put status, field paths, conclusions and correlation/request ID outside the body. Do not rebuild a JSON summary or add unrelated responses/logs. Label any necessary sanitized copy explicitly; never present it as an unchanged original.
